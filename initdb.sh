...
create_tables() {
  psql -v ON_ERROR_STOP --username "$POSTGRES_USER" <<-EOSQL
    ...
    CREATE TABLE IF NOT EXISTS results(id SERIAL PRIMARY KEY, userid VARCHAR, roomid VARCHAR, attempts JSON, total INTEGER, FOREIGN KEY(userid) REFERENCES users(username), FOREIGN KEY(roomid) REFERENCES rooms(roomid));
EOSQL
}
